...
class ExecutiveMetrics:
    ...
    def generate_daily_metrics(self, platform: str) -> Dict:
        if platform == "facebook":
            ...
            posts = self.storage.get_tt_posts(limit=10000)
            comments_count = self.storage.get_executive_summary().get("tt_comments", 0)
    ...
    def generate_insights(self, platform: str, limit: int = 10) -> List[Dict]:
        if platform == "facebook":
            ...
            posts = self.storage.get_tt_posts(limit=1000)
        ...
        for topic, counts in topic_sentiment.items():
            ...
            if neg_pct > 40:
                ...
                title = f"⚠️ Tema crítico: {topic.replace('_', ' ').title()}"
    ...
    def get_problematicas_by_zone(self, days: int = 30) -> Dict:
        ...
        if True:
            ...
            tt_posts = self.storage.get_tt_posts(limit=5000)
            ...
            for post in fb_posts + tt_posts:
                ...
                if topic:
                    problematicas.append({
                        ...
                        "zona": zona,
                        "sentiment": sentiment,
                        "platform": "fb" if post.get("post_id") else "tt"
                    })
    ...
    def get_sentiment_trend(self, platform: str, days: int = 30) -> Dict:
        metrics = self.storage.get_daily_metrics(platform=platform, days=days)
